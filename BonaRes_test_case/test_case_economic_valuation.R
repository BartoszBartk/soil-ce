...
bodium_data <- subset(bodium_data, select = c("Scenario", "Year", "Plant", "Plant_biomass_SO", "Nitrate_leach", "Corg", "AWC", "Weight"))
...
bodium_dataset$Corg <- bodium_dataset$Corg * 10000 * bodium_dataset$Weight
...
##########
# 4. Yearly economic values without discounting
##########
...
# add moments for each scenario (for comparability, reduce to 24 data points, as crop rotation length is 6 in "diverse")
...
# add moments for each scenario (24 years only, see explanation above)
val_data$mean <- rowMeans(val_data[, c(2:25)])
val_data$sd <- apply(val_data[, c(2:25)], 1, sd)

##########
# 5. Yearly economic values with discounting
##########

## Repeat analogously to above
temp_data2 <- subset(full_dataset, select = c("Scenario", "Year", "GM", "tot_wtp"))

# discount the values
r <- 0.04 # set discount rate for GM (private good)
p <- 0.01 # set discount rate to tot_wtp (public goods)
temp_data2$GM <- temp_data2$GM/(1 + r)^(temp_data2$Year - min(temp_data2$Year))
temp_data2$tot_wtp <- temp_data2$tot_wtp/(1 + p)^(temp_data2$Year - min(temp_data2$Year))

# repeat all calculations
temp_data2$tot_val <- temp_data2$GM + temp_data2$tot_wtp
temp_data2 <- temp_data2 %>% select(-c(GM, tot_wtp))
temp_data2 <- temp_data2[order(temp_data2$Year),]
val_data2 <- reshape(temp_data2, direction = "wide",
                    idvar = "Scenario",
                    timevar = "Year",
                    v.names = "tot_val")
colnames(val_data2) <- c("Scenario", c(2021:2049))
val_data2 <- val_data2[order(val_data2$Scenario),]
val_data2$NPV <- rowSums(val_data2[, c(2:25)])
val_data2$mean <- rowMeans(val_data2[, c(2:25)])
val_data2$sd <- apply(val_data2[, c(2:25)], 1, sd)

##########
# 7. Plotting
##########

## Create plots without discounting
...
# create NPV bar chart across scenarios
scenario_bars <- ggplot(val_data2, aes(x = Scenario, y = NPV)) +
